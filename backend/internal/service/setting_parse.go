...
func (s *SettingService) InitializeDefaultSettings(ctx context.Context) error {
	...
	defaults := map[string]string{
		...
		SettingKeyOpenAIUpstreamRetryCount:                           strconv.Itoa(DefaultOpenAIUpstreamRetryLimit),
...
func (s *SettingService) parseSettings(settings map[string]string) *SystemSettings {
	...
	result.OpenAIUpstreamRetryCount = DefaultOpenAIUpstreamRetryLimit
	if value, err := strconv.Atoi(strings.TrimSpace(settings[SettingKeyOpenAIUpstreamRetryCount])); err == nil && value >= 0 && value <= MaxOpenAIUpstreamRetryLimit {
		result.OpenAIUpstreamRetryCount = value
	}
	// codex_cli_only 加固
