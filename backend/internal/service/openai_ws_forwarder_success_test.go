...
func TestOpenAIGatewayService_Forward_WSv2_ChannelMappingForcesClientModelInStream(t *testing.T) {
	gin.SetMode(gin.TestMode)
	...
	captureConn := &openAIWSCaptureConn{
		events: [][]byte{
			[]byte(`{"type":"response.created","response":{"id":"resp_channel_mapped","model":"upstream-reported-model"}}`),
			[]byte(`{"type":"response.completed","response":{"id":"resp_channel_mapped","model":"upstream-reported-model","usage":{"input_tokens":2,"output_tokens":1}}}`),
		},
	}
	captureDialer := &openAIWSCaptureDialer{conn: captureConn}
	pool := newOpenAIWSConnPool(cfg)
	...
	body := []byte(`{"model":"upstream-model-b","stream":true,"input":"hello"}`)
	result, err := svc.ForwardWithOriginalModel(context.Background(), c, account, body, "client-model-a")
	require.NoError(t, err)
	require.NotNil(t, result)
	require.Equal(t, "upstream-model-b", openAIWSPayloadString(captureConn.lastWrite, "model"))
