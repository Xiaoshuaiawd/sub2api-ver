...
// HasExplicitOpenAI429RetrySignal reports whether an OpenAI 429 contains an
// explicit retry/reset time. Transport-level retry uses this to preserve the
// existing account failover behavior for cooldown-bearing 429 responses.
func HasExplicitOpenAI429RetrySignal(headers http.Header, body []byte) bool {
	return hasExplicit429RetrySignal(headers, body)
}
