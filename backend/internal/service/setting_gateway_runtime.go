...
func (s *SettingService) getGatewayForwardingSettingsCached(ctx context.Context) gatewayForwardingSettingsResult {
	if cached, ok := gatewayForwardingCache.Load().(*cachedGatewayForwardingSettings); ok && cached != nil {
		if time.Now().UnixNano() < cached.expiresAt {
			return gatewayForwardingSettingsResult{
				...
				openAIUpstreamRetryCount:         cached.openAIUpstreamRetryCount,
			}
		}
	}
	val, _, _ := gatewayForwardingSF.Do("gateway_forwarding", func() (any, error) {
		...
		values, err := s.settingRepo.GetMultiple(dbCtx, []string{
			...
			SettingKeyOpenAIUpstreamRetryCount,
		})
		if err != nil {
			...
			gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
				...
				openAIUpstreamRetryCount:         DefaultOpenAIUpstreamRetryLimit,
				expiresAt:                        time.Now().Add(gatewayForwardingErrorTTL).UnixNano(),
			})
			return gatewayForwardingSettingsResult{fp: true, claudeOAuthSystemPromptInjection: true, rewriteMessageCacheControl: s.defaultRewriteMessageCacheControl(), clientDatelineNormalization: true, openAIUpstreamRetryCount: DefaultOpenAIUpstreamRetryLimit}, nil
		}
		fp := true
		...
		openAIUpstreamRetryCount := DefaultOpenAIUpstreamRetryLimit
		if value, err := strconv.Atoi(strings.TrimSpace(values[SettingKeyOpenAIUpstreamRetryCount])); err == nil && value >= 0 && value <= MaxOpenAIUpstreamRetryLimit {
			openAIUpstreamRetryCount = value
		}
		gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
			...
			openAIUpstreamRetryCount:         openAIUpstreamRetryCount,
	...
	return gatewayForwardingSettingsResult{fp: true, claudeOAuthSystemPromptInjection: true, clientDatelineNormalization: true, openAIUpstreamRetryCount: DefaultOpenAIUpstreamRetryLimit}
}
...
// GetOpenAIUpstreamRetryCount returns the cached request retry count for OpenAI HTTP upstreams.
func (s *SettingService) GetOpenAIUpstreamRetryCount(ctx context.Context) int {
	return s.getGatewayForwardingSettingsCached(ctx).openAIUpstreamRetryCount
}
