...
// ClearOpsUpstreamErrorForRetry removes an intermediate upstream failure before
// retrying the same downstream request. Only the final exhausted failure should
// be persisted by OpsErrorLoggerMiddleware.
func ClearOpsUpstreamErrorForRetry(c *gin.Context) {
	if c == nil {
		return
	}
