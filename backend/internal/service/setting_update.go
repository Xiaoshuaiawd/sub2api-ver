...
func (s *SettingService) buildSystemSettingsUpdates(ctx context.Context, settings *SystemSettings) (map[string]string, error) {
	...
	if settings.OpenAIUpstreamRetryCount < 0 || settings.OpenAIUpstreamRetryCount > MaxOpenAIUpstreamRetryLimit {
		return nil, infraerrors.BadRequest(
			"INVALID_OPENAI_UPSTREAM_RETRY_COUNT",
			fmt.Sprintf("openai_upstream_retry_count must be between 0 and %d", MaxOpenAIUpstreamRetryLimit),
		)
	}
	...
	updates[SettingKeyOpenAIUpstreamRetryCount] = strconv.Itoa(settings.OpenAIUpstreamRetryCount)
	// codex_cli_only 加固
...
func (s *SettingService) refreshCachedSettings(settings *SystemSettings) {
	...
	gatewayForwardingCache.Store(&cachedGatewayForwardingSettings{
		...
		openAIUpstreamRetryCount:         settings.OpenAIUpstreamRetryCount,
