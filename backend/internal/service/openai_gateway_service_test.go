...
func TestReplaceModelInResponseBody(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:     "同时替换顶层和嵌套 model",
			body:     `{"model":"gpt-4o","response":{"model":"gpt-4o"}}`,
			from:     "gpt-4o",
			to:       "alias",
			expected: `{"model":"alias","response":{"model":"alias"}}`,
		},
	}
