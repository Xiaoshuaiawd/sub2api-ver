...
func TestSchedulerCacheSnapshotL1ReusesActiveVersion(t *testing.T) {
	...
	require.Equal(t, "first", first[0].Name)

	mutated := account
	mutated.Name = "redis-only-change"
	metadata, err := json.Marshal(buildSchedulerMetadataAccount(mutated))
	require.NoError(t, err)
	require.NoError(t, cache.rdb.Set(ctx, schedulerAccountMetaKey("9011"), metadata, 0).Err())
...
func TestSchedulerCacheSnapshotL1ReloadsAfterAccountUpdate(t *testing.T) {
	ctx := context.Background()
	cache, _ := newSchedulerCacheUnitWithRedis(t)
	bucket := service.SchedulerBucket{GroupID: 904, Platform: service.PlatformOpenAI, Mode: service.SchedulerModeSingle}
	account := service.Account{
		ID:          9041,
		Name:        "first",
		Platform:    service.PlatformOpenAI,
	...
	require.NoError(t, err)
	require.True(t, hit)
	require.Equal(t, "updated", loaded[0].Name)
}

func TestSchedulerCacheSnapshotL1ReturnsIsolatedAccounts(t *testing.T) {
	ctx := context.Background()
	cache, _ := newSchedulerCacheUnitWithRedis(t)
	bucket := service.SchedulerBucket{GroupID: 905, Platform: service.PlatformOpenAI, Mode: service.SchedulerModeSingle}
	lastUsed := time.Now().Add(-time.Hour).UTC().Truncate(time.Millisecond)
	account := service.Account{
		ID:          9051,
		Name:        "stable",
		Platform:    service.PlatformOpenAI,
		Type:        service.AccountTypeOAuth,
		Credentials: map[string]any{"model_mapping": map[string]any{"alias": "upstream"}},
		Status:      service.StatusActive,
		Schedulable: true,
		Concurrency: 1,
		LastUsedAt:  &lastUsed,
	...
	require.NoError(t, err)
	require.True(t, hit)
	first[0].Credentials["model_mapping"].(map[string]any)["alias"] = "mutated"
	first[0].GroupIDs[0] = 999
	*first[0].LastUsedAt = lastUsed.Add(time.Hour)

	second, hit, err := cache.GetSnapshot(ctx, bucket)
	require.NoError(t, err)
	require.True(t, hit)
	require.Equal(t, "upstream", second[0].Credentials["model_mapping"].(map[string]any)["alias"])
	require.Equal(t, []int64{905}, second[0].GroupIDs)
	require.Equal(t, lastUsed, *second[0].LastUsedAt)
}

func TestSchedulerCacheSnapshotL1ClearsWhenReadyKeyDisappears(t *testing.T) {
	ctx := context.Background()
	cache, _ := newSchedulerCacheUnitWithRedis(t)
	bucket := service.SchedulerBucket{GroupID: 906, Platform: service.PlatformOpenAI, Mode: service.SchedulerModeSingle}
	account := service.Account{
		ID:          9061,
		Name:        "first",
		Platform:    service.PlatformOpenAI,
	...
	account.Name = "updated"
	metadata, err := json.Marshal(buildSchedulerMetadataAccount(account))
	require.NoError(t, err)
	require.NoError(t, cache.rdb.Set(ctx, schedulerAccountMetaKey("9061"), metadata, 0).Err())
	readyKey := schedulerBucketKey(schedulerReadyPrefix, bucket)
	require.NoError(t, cache.rdb.Del(ctx, readyKey).Err())
	_, hit, err = cache.GetSnapshot(ctx, bucket)
	require.NoError(t, err)
	require.False(t, hit)
	require.NoError(t, cache.rdb.Set(ctx, readyKey, "1", 0).Err())
...
func TestSchedulerCacheSnapshotL1DoesNotReplaceNewerVersion(t *testing.T) {
	cache := newSchedulerCacheUnit(t)
	bucket := service.SchedulerBucket{GroupID: 907, Platform: service.PlatformOpenAI, Mode: service.SchedulerModeSingle}
	cache.storeSnapshotL1(bucket, schedulerSnapshotL1Entry{
		version:  "2",
		accounts: []*service.Account{{ID: 9072, Name: "newer"}},
	})
	cache.storeSnapshotL1(bucket, schedulerSnapshotL1Entry{
		version:  "1",
		accounts: []*service.Account{{ID: 9071, Name: "older"}},
	})

	loaded, ok := cache.getSnapshotL1(bucket, "2", "")
	require.True(t, ok)
	require.Equal(t, "newer", loaded.accounts[0].Name)
}

func TestSchedulerCacheSnapshotL1DoesNotReplaceNewerLastUsedRefresh(t *testing.T) {
	cache := newSchedulerCacheUnit(t)
	bucket := service.SchedulerBucket{GroupID: 908, Platform: service.PlatformOpenAI, Mode: service.SchedulerModeSingle}
	newerRefresh := time.Now()
	cache.storeSnapshotL1(bucket, schedulerSnapshotL1Entry{
		version:             "1",
		metadataRevision:    "1",
		accounts:            []*service.Account{{ID: 9081, Name: "newer"}},
		lastUsedRefreshedAt: newerRefresh,
	})
	cache.storeSnapshotL1(bucket, schedulerSnapshotL1Entry{
		version:             "1",
		metadataRevision:    "1",
		accounts:            []*service.Account{{ID: 9081, Name: "older"}},
		lastUsedRefreshedAt: newerRefresh.Add(-time.Second),
	})

	loaded, ok := cache.getSnapshotL1(bucket, "1", "1")
	require.True(t, ok)
	require.Equal(t, "newer", loaded.accounts[0].Name)
}

func TestSchedulerCacheSnapshotL1ReloadsAfterActiveVersionChanges(t *testing.T) {
	ctx := context.Background()
	cache, _ := newSchedulerCacheUnitWithRedis(t)
	bucket := service.SchedulerBucket{GroupID: 902, Platform: service.PlatformOpenAI, Mode: service.SchedulerModeSingle}
	...
	token, err := cache.CaptureBucketWriteToken(ctx, bucket)
	require.NoError(t, err)
	require.NoError(t, cache.SetSnapshot(ctx, bucket, token, []service.Account{first}))
	_, hit, err := cache.GetSnapshot(ctx, bucket)
	require.NoError(t, err)
	require.True(t, hit)
	...
	second := first
	second.Name = "second"
	nextToken, err := cache.CaptureBucketWriteToken(ctx, bucket)
	require.NoError(t, err)
	require.NoError(t, cache.SetSnapshot(ctx, bucket, nextToken, []service.Account{second}))
...
func TestSchedulerCacheSnapshotL1RefreshesLastUsedSideKeys(t *testing.T) {
	ctx := context.Background()
	cache, _ := newSchedulerCacheUnitWithRedis(t)
	bucket := service.SchedulerBucket{GroupID: 903, Platform: service.PlatformOpenAI, Mode: service.SchedulerModeSingle}
	initial := time.Now().Add(-time.Hour).UTC().Truncate(time.Millisecond)
	account := service.Account{
		ID:          9031,
	...
	latest := initial.Add(30 * time.Minute)
	require.NoError(t, cache.UpdateLastUsed(ctx, map[int64]time.Time{account.ID: latest}))
	key := schedulerBucketKey("snapshot-l1:", bucket)
	cache.snapshotL1Mu.Lock()
	entry := cache.snapshotL1[key]
	entry.lastUsedRefreshedAt = time.Now().Add(-2 * defaultSchedulerLastUsedL1TTL)
	cache.snapshotL1[key] = entry
	cache.snapshotL1Mu.Unlock()
	...
	require.True(t, hit)
	require.Len(t, loaded, 1)
	require.Equal(t, "stable-metadata", loaded[0].Name)
	require.NotNil(t, loaded[0].LastUsedAt)
	require.Equal(t, latest, *loaded[0].LastUsedAt)
}
...
func TestSchedulerCacheSetAccountAdvancesRevisionOnlyForMetadataChanges(t *testing.T) {
	ctx := context.Background()
	cache, _ := newSchedulerCacheUnitWithRedis(t)
	account := service.Account{
		ID:          115,
		...
		Platform:    service.PlatformOpenAI,
		Type:        service.AccountTypeOAuth,
		Credentials: map[string]any{"access_token": "first"},
		Status:      service.StatusActive,
		Schedulable: true,
	}
	require.NoError(t, cache.SetAccount(ctx, &account))
	require.Equal(t, "1", cache.rdb.Get(ctx, schedulerAccountMetaRevisionKey).Val())

	account.Credentials["access_token"] = "refreshed"
	require.NoError(t, cache.SetAccount(ctx, &account))
	require.Equal(t, "1", cache.rdb.Get(ctx, schedulerAccountMetaRevisionKey).Val())

	account.Name = "changed"
	require.NoError(t, cache.SetAccount(ctx, &account))
	require.Equal(t, "2", cache.rdb.Get(ctx, schedulerAccountMetaRevisionKey).Val())
}
