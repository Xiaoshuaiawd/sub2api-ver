...
type GatewayConfig struct {
	...
	// OpenAIRequestMaxDurationSeconds: 单个 OpenAI 请求的总时长上限（秒）。超过上限即截断：
	// 取消上游请求，已下发的流式内容保持原样，客户端看到流提前结束。
	// 0 表示禁用；默认 600（10 分钟）。
	OpenAIRequestMaxDurationSeconds int `mapstructure:"openai_request_max_duration_seconds"`
...
func setDefaults() {
	...
	viper.SetDefault("gateway.openai_request_max_duration_seconds", 600)
...
func (c *Config) Validate() error {
	...
	if c.Gateway.OpenAIRequestMaxDurationSeconds < 0 || c.Gateway.OpenAIRequestMaxDurationSeconds > 86400 ||
		(c.Gateway.OpenAIRequestMaxDurationSeconds > 0 && c.Gateway.OpenAIRequestMaxDurationSeconds < 30) {
		return fmt.Errorf("gateway.openai_request_max_duration_seconds must be 0 or between 30-86400 seconds")
	}
