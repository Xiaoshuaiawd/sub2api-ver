...
func (h *OpenAIGatewayHandler) Images(c *gin.Context) {
	...
	for {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &imageUpstreamErr) {
					...
					h.gatewayService.ReportOpenAIAccountSelectionResult(selection, account.GetMappedModel(requestModel), !retryableServerError, nil)
				...
				if errors.As(err, &failoverErr) {
					h.gatewayService.ReportOpenAIAccountSelectionResult(selection, account.GetMappedModel(requestModel), false, nil)
		...
		if result != nil {
			...
			h.gatewayService.ReportOpenAIAccountSelectionResult(selection, account.GetMappedModel(requestModel), true, result.FirstTokenMs)
		} else {
			h.gatewayService.ReportOpenAIAccountSelectionResult(selection, account.GetMappedModel(requestModel), true, nil)
		}
