...
type UpdateSettingsRequest struct {
	...
	OpenAIUpstreamRetryCount               *int    `json:"openai_upstream_retry_count"`
...
func (h *SettingHandler) UpdateSettings(c *gin.Context) {
	...
	if req.OpenAIUpstreamRetryCount != nil && (*req.OpenAIUpstreamRetryCount < 0 || *req.OpenAIUpstreamRetryCount > service.MaxOpenAIUpstreamRetryLimit) {
		response.Error(c, http.StatusBadRequest, fmt.Sprintf("openai_upstream_retry_count must be between 0 and %d", service.MaxOpenAIUpstreamRetryLimit))
		return
	}
	...
	settings := &service.SystemSettings{
		...
		OpenAIUpstreamRetryCount: func() int {
			if req.OpenAIUpstreamRetryCount != nil {
				return *req.OpenAIUpstreamRetryCount
			}
			return previousSettings.OpenAIUpstreamRetryCount
		}(),
	...
	payload := dto.SystemSettings{
		...
		OpenAIUpstreamRetryCount:                               updatedSettings.OpenAIUpstreamRetryCount,
