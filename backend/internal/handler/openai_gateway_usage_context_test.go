...
func TestSubmitUsageRecordTaskCopiesRequestContext(t *testing.T) {
	...
	capture := service.NewMessageCaptureSession(service.MessageCaptureConfig{MaxBodyBytes: 1024, MemoryBudgetBytes: 4096})
	parent = service.WithMessageCapture(parent, capture)

	var gotClientRequestID string
	var gotRequestID string
	var gotCapture *service.MessageCaptureSession
	h := &GatewayHandler{}
	h.submitUsageRecordTask(parent, func(ctx context.Context) {
		gotClientRequestID, _ = ctx.Value(ctxkey.ClientRequestID).(string)
		gotRequestID, _ = ctx.Value(ctxkey.RequestID).(string)
		gotCapture = service.MessageCaptureFromContext(ctx)
	})
