export default {
    settings: {
      ...
      gatewayForwarding: {
        ...
        openaiProxyTitle: 'OpenAI 默认出口代理',
        openaiProxyDescription: '仅作用于 OpenAI、ChatGPT 与 Codex 上游请求。账号代理优先；没有账号代理或代理链失败时，才使用当前节点的默认代理。',
        openaiProxyEnabled: '启用节点默认代理',
        openaiProxyEnabledHint: '开启后，每台 Sub2API 节点使用自己的默认代理出口。',
        openaiProxyURL: '代理地址',
        openaiProxyURLPlaceholder: 'socks5h://warp-proxy:1080',
        openaiProxyURLConfiguredPlaceholder: '代理凭证已配置，输入新地址可替换',
        openaiProxyURLHint: '支持 HTTP、HTTPS、SOCKS5 与 SOCKS5H；WARP 建议使用 socks5h://warp-proxy:1080。',
        openaiProxyFailurePolicy: '代理失败策略',
        openaiProxyFailClosed: '失败时阻断请求',
        openaiProxyFailClosedHint: '所有代理候选不可用时返回错误，不会绕过代理直连上游。',
        openaiProxyFallbackDirect: '失败后允许直连',
        openaiProxyFallbackDirectHint: '所有代理候选不可用时，最后尝试从当前节点直接连接上游。',
        openaiProxyFallbackDirectWarning: '允许直连会暴露当前节点公网 IP，仅在接受此风险时启用。',
        openaiProxyStatusTitle: '当前节点状态',
        openaiProxyHealthy: '代理出口正常',
        openaiProxyUnhealthy: '代理出口异常',
        openaiProxyPending: '等待首次检测',
        openaiProxyInstance: '节点',
        openaiProxyEgressIP: '出口 IP',
        openaiProxyCheckedAt: '检测时间',
        openaiProxyUnavailable: '暂无',
        ...
        openaiCodexUpstreamURL: 'Codex 上游接口地址',
        openaiCodexUpstreamURLPlaceholder: '留空使用官方 https://chatgpt.com/backend-api/codex/responses',
        openaiCodexUpstreamURLHint: '配置后 Codex（OAuth 账号）的 Responses 请求会原样改打到该地址：请求体、请求头与鉴权保持不变，仅目标 URL 不同；HTTP 与 WebSocket 同时生效。留空则使用官方地址。',
